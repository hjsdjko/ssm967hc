      ...
      <el-col :span="24">  
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.cantingtupian" label="餐厅图片" prop="cantingtupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.cantingtupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  data() {
	...
	jingyingfanwei : false,
      ...
      rules: {
          cantingbianhao: [
                { required: true, message: '餐厅编号不能为空', trigger: 'blur' },
          ],
          mima: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `canting/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.cantingCrossAddOrUpdateFlag = false;
						 this.parent.search();
