<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="配送姓名" prop="peisongxingming">
          <el-input v-model="ruleForm.peisongxingming" 
              placeholder="配送姓名" clearable  :readonly="ro.peisongxingming"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="配送姓名" prop="peisongxingming">
              <el-input v-model="ruleForm.peisongxingming" 
                placeholder="配送姓名" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  data() {
	...
	dianhua : false,
      ...
      rules: {
          peisonggonghao: [
                { required: true, message: '配送工号不能为空', trigger: 'blur' },
          ],
          mima: [
          ],
          peisongxingming: [
                { required: true, message: '配送姓名不能为空', trigger: 'blur' },
          ],
          xingbie: [
          ],
          zhaopian: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `waimaiyuan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.waimaiyuanCrossAddOrUpdateFlag = false;
						 this.parent.search();
